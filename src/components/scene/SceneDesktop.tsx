...
export function SceneDesktop() {
  ...
  const single = moments.find((m) => m.id === singleId)!;
  ...
  const pickForCompare = (m: Moment) => {
    ...
    // replace the older-selected slot: swap into right, push right→left
    setLeftId(rightId);
    setRightId(m.id);
  };

  return (
    <div className="scheme-a min-h-screen bg-background text-foreground">
      <TopBar />
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 py-6">
        <div className="grid gap-6 lg:grid-cols-[1fr_1.35fr]">
          ...
          <section className="lg:sticky lg:top-20 self-start">
            <MapPanel activeSceneId={defaultScene.id} />
          </section>
          ...
          <section>
            <header className="mb-5">
              ...
              <p className="mt-2 text-sm text-muted-foreground inline-flex flex-wrap items-center gap-x-4 gap-y-1">
                <span className="inline-flex items-center gap-1">
                  <Compass size={12} /> {defaultScene.direction}
                </span>
                <span className="inline-flex items-center gap-1">
                  <Users size={12} /> {defaultScene.momentCount} 个时刻，{defaultScene.momentCount} 位记录者
                </span>
              </p>
            </header>
            ...
            >
              <ModeTab active={mode === "single"} onClick={() => setMode("single")}>单张浏览</ModeTab>
              <ModeTab active={mode === "compare"} onClick={() => setMode("compare")}>双图比较</ModeTab>
              <ModeTab active={mode === "all"} onClick={() => setMode("all")}>全部时刻</ModeTab>
            </div>
            ...
            {mode === "single" && (
              <>
                ...
                <div className="overflow-hidden rounded-[8px] border border-border bg-muted" style={{ aspectRatio: "3 / 2" }}>
                  <img
                    src={single.imageUrl}
            ...
            {mode === "all" && (
              <div className="space-y-8">
                {moments.map((m) => (
                  <article key={m.id}>
                    <MomentHeader m={m} />
                    <div className="mt-3 overflow-hidden rounded-[8px] border border-border bg-muted" style={{ aspectRatio: "3 / 2" }}>
                      <img
                        src={m.imageUrl}
                        alt={m.shortLabel}
                        loading="lazy"
...
function ModeTab({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`h-8 whitespace-nowrap rounded-[6px] px-3 transition-colors ${
        active
    ...
    <header className="sticky top-0 z-20 border-b border-border bg-background/85 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-6xl items-center gap-3 px-4 sm:px-6">
        <Link to="/" className="inline-flex items-center gap-2">
          ...
          <span className="hidden sm:inline text-xs text-muted-foreground">同一个视角，不同的时刻</span>
        </Link>
        <nav className="ml-auto flex items-center gap-1 text-sm text-muted-foreground">
          ...
          <Link
            to="/schemes"
            className="hidden sm:inline-flex h-9 items-center gap-1 rounded-[8px] px-2 hover:text-foreground"
          >
            方案对比 <ChevronRight size={13} />
          </Link>
        </nav>
      </div>
    </header>
...
function MapPanel({ activeSceneId }: { activeSceneId: string }) {
  return (
    <div>
      <div
      ...
      >
        ...
        {scenes.map((s) => {
          ...
          // Deterministic layout inside faux map viewbox
            ...
            >
              <span className="flex flex-col items-center">
                ...
                >
                  ...
                  {s.title}
                  <span className="text-muted-foreground">· {s.momentCount}</span>
                </span>
                <span
      ...
      <div className="mt-3 rounded-[8px] border border-border bg-surface p-3 text-xs text-muted-foreground leading-6">
        ...
        <p className="mt-2">
          当前视角：<span className="text-foreground">{defaultScene.title}</span>
        </p>
      </div>
      <Link
        to="/upload"
        search={{ from: "scene", sceneId: "shiqiao-ne" }}
        className="mt-3 inline-flex h-10 w-full items-center justify-center gap-1.5 rounded-[8px] bg-accent text-[var(--accent-foreground)] text-sm hover:opacity-90"
      >
...
function FauxLakeMap() {
  ...
  return (
    <svg viewBox="0 0 400 300" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-hidden>
      ...
      {/* lake */}
      <path d="M 60 150 Q 120 90 220 100 T 380 170 L 380 300 L 60 300 Z" fill="rgba(181,58,43,0.05)" />
      {/* shoreline */}
      <path d="M 60 150 Q 120 90 220 100 T 380 170" stroke="rgba(0,0,0,0.25)" strokeWidth="1.2" fill="none" />
      {/* road along shore */}
      <path d="M 40 165 Q 130 110 230 118 T 380 190" stroke="rgba(181,58,43,0.55)" strokeWidth="1.4" fill="none" strokeDasharray="4 3" />
      {/* street grid on land */}
      {Array.from({ length: 6 }).map((_, i) => (
        <line key={`h${i}`} x1="0" x2="400" y1={i * 22 + 10} y2={i * 22 + 10} stroke="rgba(0,0,0,0.06)" />
      ))}
